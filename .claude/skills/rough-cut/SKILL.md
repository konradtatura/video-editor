...
**A full review console (`scripts/review_console.py`) goes one step further: an actual page with the video, a clickable timeline, a flag button, and direct boundary trimming.**
```
python scripts/review_console.py projects/<name>          # -> review.html, embeds output.mp4 by default
```
Serve it with the `review-console` config in `.claude/launch.json` (`range_server.py`, see below -- not `python -m http.server`) and open `http://localhost:<port>/<project_name>/review.html`. The user watches, clicks a flag type (pauses the video, captures the exact time), optionally adds a short note, and hits "copy flags for chat" to paste a ready-made list back -- e.g. `- 0:18.2 (clip 3) [repeat]: upselle said twice`. Use this for coarse/structural notes.

**Dragging a clip's left/right edge directly on the timeline trims it live** -- each clip has a small grab handle at its start and end; dragging resizes that clip's block in real time with a floating label showing the new raw-footage time, and releasing commits the change (POST `/api/trim`) and triggers a re-render. **Clicking the middle of a clip instead opens a fine-tune panel**: raw start/end as editable numbers, a raw-footage preview video to scrub for the right spot, "set = preview time" to capture a boundary precisely, and "Save & re-render" -- useful when you want a specific number or need to see surrounding raw footage the current cut has already excluded, rather than eyeballing a drag. Both paths write through the same `/api/trim` endpoint. **First version of this only had the fine-tune panel; confirmed directly that wasn't what the user meant by "just like cropping in capcut" -- they expected to drag the timeline blocks themselves**, so drag-to-trim is the primary interaction and the panel is the secondary/precise one, not the other way around.

This writes directly through `cutlist.json` -- `/api/trim` (handled by `range_server.py`) edits that clip's `start`/`end` in the file and shells out to `render.py` (cache-aware, so only the changed segment re-encodes, a few seconds not a full re-render), then the page updates the video and timeline with the new cut in place. This is not a UI-only preview -- `cutlist.json` stays the single source of truth, it's just edited from the browser now instead of Claude relaying a timestamp back into the file by hand. **Confirmed user preference: for fine boundary placement ("slightly move chunks according to my eyes and ears"), direct manipulation beats describing a cut point in words** -- prose feedback has no guarantee of landing exactly right and costs a round trip either way; the flag-list flow above is still the right tool for coarse/structural notes ("a repeat is left in", "this whole clip should go"), not fine nudges.

Real bugs found and fixed building this and its trim extension, worth knowing if this ever needs touching again:
- **A handle's CSS class and the semantic edge name it represents must actually match, or the drag silently no-ops.** First version named the handle divs' CSS classes `left`/`right` (visual position) but passed `'start'`/`'end'` (semantic field name) into the drag handler, which then queried `.tl-handle.start`/`.tl-handle.end` -- neither ever matched, so `querySelector` returned `null` and the very next line (`handle.classList.add(...)`) threw, caught nowhere, silently killing the whole drag with zero visible symptom beyond "nothing happens." Caught only by dispatching synthetic mouse events in a test and explicitly capturing `window.onerror` -- a normal click-and-drag test in a real browser would have shown the same "doesn't work" symptom without the stack trace pointing at the exact line. Fixed by using one vocabulary (`start`/`end`) for both the CSS class and the semantic parameter, everywhere. Lesson: when a name has to agree across two places (a CSS class and a JS string, an HTML attribute and a query selector), use the literal same string in both, not two parallel names for the same concept that happen to be synonyms.
- **Adjacent clips' edge-handles must not overlap in screen space.** The output timeline is contiguous (clip N's `output_end` equals clip N+1's `output_start`, even though their raw-footage ranges have a gap between them), so a handle positioned to protrude *past* its own clip's edge (e.g. `left: -5px`) lands on the exact same pixels as the neighboring clip's handle on the other side of that boundary -- whichever clip is later in DOM order wins every time, silently making the other clip's edge on that side ungrabbable. Fixed by keeping each handle fully inside its own clip's box (`left: 0` / `right: 0`) instead of straddling the border.
...
- **`/api/trim`'s `render.py` subprocess is launched via `sys.executable`** (whatever interpreter is running `range_server.py`), so `.claude/launch.json`'s `review-console` config must point at the venv's Python (`venv-whisper/Scripts/python.exe` on Windows), not a bare system `python` -- `render.py` imports `audio_boundaries.py`, which needs `numpy`/`scipy`. A bare system Python here means every trim save fails with an `ImportError` inside the subprocess, with the failure surfacing only in the JSON error response, not at server-start time.
- **`/api/trim` is the one write-capable endpoint on an otherwise read-only static server** -- it validates the `project` name has no path separators and resolves to a direct child of the served directory before touching any file, since path-traversal here would mean writing/executing outside `projects/`. Still local-only, no auth beyond that; fine for a personal review tool, worth remembering before ever exposing this server beyond localhost.
...
- `scripts/review_console.py` -- generates `review.html`: video + clickable timeline (clip boundaries from `timeline.json`/`cutlist.json`) + a flag-recording panel that exports a paste-ready coordinate list + a per-clip trim panel (raw-footage preview, editable start/end, "Save & re-render") that writes through `cutlist.json` via `range_server.py`'s `/api/trim`. See "A full review console..." above for the serving bugs (no Range support, missing `+faststart`, wrong Python in `launch.json`) this needed to actually work in a browser.
- `scripts/range_server.py` -- small static file server with actual HTTP Range-request support (threaded), for serving video to `review.html`. Python's stdlib `http.server` has none at all -- use this instead, via `.claude/launch.json`'s `review-console` config. Also handles `POST /api/trim`, the one write-capable endpoint: edits a project's `cutlist.json` and re-renders (via `render.py`, run under the same interpreter -- must be the venv's Python, see above).
