...
**Clicking a clip on the timeline opens a trim panel: raw start/end as editable numbers, a raw-footage preview video to scrub for the right spot, "set = preview time" to capture a boundary, and "Save & re-render."** This writes directly through `cutlist.json` -- POST `/api/trim` (handled by `range_server.py`) edits that clip's `start`/`end` in the file and shells out to `render.py` (cache-aware, so only the changed segment re-encodes, a few seconds not a full re-render), then the page reloads the video and the timeline with the new cut. This is not a UI-only preview -- `cutlist.json` stays the single source of truth, it's just edited from the browser now instead of Claude relaying a timestamp back into the file by hand. **Confirmed user preference: for fine boundary placement ("slightly move chunks according to my eyes and ears"), direct trimming beats describing a cut point in words** -- prose feedback has no guarantee of landing exactly right and costs a round trip either way; the flag-list flow above is still the right tool for coarse/structural notes ("a repeat is left in", "this whole clip should go"), not fine nudges.
